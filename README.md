...
## Pre-commit проверки списков

В репозитории есть `pre-commit` хук (`.githooks/pre-commit`) с проверками `lists/*.txt`:

- не более 300 строк в каждом `*.txt`;
- один и тот же домен не может быть в разных файлах;
- если уже есть домен более высокого уровня (например, `b.a`), нельзя добавлять поддомен (`c.b.a`);
- если уже есть более широкая сеть (`10.0.0.0/8`), нельзя добавлять более узкую (`10.10.0.0/24`).

Включить хуки для этого репозитория:

```bash
git config core.hooksPath .githooks
```

Собрать отчёт по текущим пересечениям в списках:

```bash
python3 scripts/make_overlap_report.py
```

Отсортировать списки (`домены -> IP/CIDR`):

```bash
python3 scripts/sort_lists.py
```

Отсортировать только выбранные файлы:

```bash
python3 scripts/sort_lists.py lists/Gemini.txt lists/youtube.txt
```
